...
STATIC_USP = """\n\nТы слышал голосовое — я слов на ветер не бросаю. Забудь про мутные конторы и агентов.

Здесь работаю только я. Я — Хозяин, я — Автор, я — Гарант.
— Нет испорченного телефона через менеджеров.
— Нет переплат за штат дармоедов.
— Есть только моя личная ответственность за твою оценку.

Скидывай задачу — я всё сделаю красиво. Жми кнопку внизу, не стесняйся."""
...
def get_time_greeting() -> str:
    ...
    if 6 <= msk_hour < 12:
        return GREETING_MORNING + STATIC_USP
    elif 12 <= msk_hour < 18:
        return GREETING_DAY + STATIC_USP
    elif 18 <= msk_hour < 24:
        return GREETING_EVENING + STATIC_USP
    else:  # 0 <= msk_hour < 6
        return GREETING_NIGHT + STATIC_USP
